...
__host__ void init_host_matrices(half *a, half *b, int M_GLOBAL, int K_GLOBAL, int N_GLOBAL, int MATRIX_A_PRUNING_PERCENTAGE) {
...
void PrintMismatch(const char *KernelName, int MaxNumMismatch, float ErrorThreshold, half *CuBlas, half *Other, int M_GLOBAL, int N_GLOBAL) {
    ...
    for (int i = 0; i < M_GLOBAL; i++) {
        for (int j = 0; j < N_GLOBAL; j++) {
            if (fabs(__half2float(CuBlas[i + j * M_GLOBAL]) - __half2float(Other[i + j * M_GLOBAL])) > ErrorThreshold) {
                count++;
                printf("(%d,%d) CuBlas=%f %s=%f\n", i, j, __half2float(CuBlas[i + j * M_GLOBAL]), KernelName, __half2float(Other[i + j * M_GLOBAL]));
            }
...
void PrintMismatchV1(const char *KernelName, int MaxNumMismatch, float ErrorThreshold, half *CuBlas, half *Other, int M_GLOBAL, int N_GLOBAL) {
    printf("First 10 Mismatches between Cublas and %s:\n", KernelName);
    int count = 0;
    for (int i = 0; i < M_GLOBAL; i++) {
        for (int j = 0; j < N_GLOBAL; j++) {
            if (fabs(__half2float(CuBlas[i * N_GLOBAL + j]) - __half2float(Other[i * N_GLOBAL + j])) > ErrorThreshold) {
                count++;
                printf("(%d,%d) CuBlas=%f %s=%f\n", i, j, __half2float(CuBlas[i * N_GLOBAL + j]), KernelName, __half2float(Other[i * N_GLOBAL + j]));
            }
            if (count == MaxNumMismatch)
                break;
        }
...
void PrintMatrix(const char *MatrixName, half *Matrix, int M_GLOBAL, int N_GLOBAL, int MaxRows, int MaxCols, int start_row, int start_col) {
...
void PrintMatrixVec(const char *MatrixName, half *Matrix, int M_GLOBAL, int N_GLOBAL, int MaxRows, int MaxCols) {
...
void SavePerformanceData(const char *filename, int M, int K, int N, int SplitK, int Sparsity, float duration_cublas_tc, float tflops_cublas_tc,
                         float duration_SpMM2, float tflops_SpMM2, float duration_SpMM_bitmapv3, float tflops_SpMM_bitmapv3) {
    FILE *fp;
    ...
    fprintf(fp, "%d,%d,%d,%d,%d,%s,%.1f,%.5f\n", M, K, N, SplitK, Sparsity, "SpInfer", duration_SpMM_bitmapv3_ns, tflops_SpMM_bitmapv3);

    fprintf(fp, "%d,%d,%d,%d,%d,%s,%.1f,%.5f\n", M, K, N, SplitK, Sparsity, "cuBLAS_TC", duration_cublas_tc_ns, tflops_cublas_tc);

    fprintf(fp, "%d,%d,%d,%d,%d,%s,%.1f,%.5f\n", M, K, N, SplitK, Sparsity, "Flash-LLM", duration_SpMM2_ns, tflops_SpMM2);

    fclose(fp);
}

void SaveCuSparsePerformanceData(const char *filename, int M, int K, int N, int SplitK, int Sparsity, float duration_CuSparse_ColMajor,
    ...
    fprintf(fp, "%d,%d,%d,%d,%d,%s,%.1f,%.5f\n", M, K, N, SplitK, Sparsity, "cuSPARSE", cusparse_duration_ns, cusparse_tflops);

    fclose(fp);
}
void SaveSputnikPerformanceData(const char *filename, int M, int K, int N, int SplitK, int Sparsity, float duration_Sputnik, float tflops_Sputnik) {
    FILE *fp;
    // Try to open file to check if it exists
    fp = fopen(filename, "r");
    bool fileExists = (fp != NULL);
    ...
    fprintf(fp, "%d,%d,%d,%d,%d,%s,%.1f,%.5f\n", M, K, N, SplitK, Sparsity, "Sputnik", sputnik_duration_ns, tflops_Sputnik);

    fclose(fp);
}
void SaveSparTAPerformanceData(const char *filename, int M, int K, int N, int SplitK, int Sparsity, float duration_sparTA, float tflops_sparTA) {
    FILE *fp;
    // Try to open file to check if it exists
    fp = fopen(filename, "r");
    bool fileExists = (fp != NULL);
    ...
    fprintf(fp, "%d,%d,%d,%d,%d,%s,%.1f,%.5f\n", M, K, N, SplitK, Sparsity, "SparTA", sparta_duration_ns, tflops_sparTA);
...
int InitSparseMatrixA_bitmap_v6(half *A_h, int M, int K,
    ...
    *bitmap = (uint64_t *)malloc(num_tiles * sizeof(uint64_t));
    ...
    if (*Compressed_Val == nullptr || *TileOffsets == nullptr || *TileOffsets_median == nullptr || *TileOffsets_global == nullptr ||
        *bitmap == nullptr) {
        return -1;
    }
...
void print_bitmap_v3_results(half *Compressed_Val, int *TileOffsets, int *TileOffsets_global, uint64_t *bitmap, int num_tiles, int num_global_tiles,
                             int val_count) {
...
void print_bitmap_v6_results(half *Compressed_Val, int *TileOffsets, int *TileOffsets_median, int *TileOffsets_global, uint64_t *bitmap,
                             int num_tiles, int num_median_tiles, int num_global_tiles, int val_count) {
    std::cout << "TileOffsets: ";
    for (int i = 0; i < num_tiles; ++i) {
        std::cout << "-i: " << i << " " << TileOffsets[i] << " ";
    }
