...
__host__ void init_host_matrices(half *a, half *b, int M_GLOBAL, int K_GLOBAL, int N_GLOBAL,
                                 int MATRIX_A_PRUNING_PERCENTAGE)
{
...
double ComputeTotalVal(half *CuBlas, half *Other, int m, int n)
{
    double totalError = 0.0;
    for (int i = 0; i < m * n; i++)
        totalError += __half2float(CuBlas[i]) ;
    return totalError;
}

void PrintMismatch(const char *KernelName, int MaxNumMismatch, float ErrorThreshold, half *CuBlas,
                   half *Other, int M_GLOBAL, int N_GLOBAL)
{
    ...
    for (int i = 0; i < M_GLOBAL; i++)
    {
        for (int j = 0; j < N_GLOBAL; j++)
        {
            if (fabs(__half2float(CuBlas[i + j * M_GLOBAL]) -
                     __half2float(Other[i + j * M_GLOBAL])) > ErrorThreshold)
            {
                count++;
                printf("(%d,%d) CuBlas=%f %s=%f\n", i, j, __half2float(CuBlas[i + j * M_GLOBAL]),
                       KernelName, __half2float(Other[i + j * M_GLOBAL]));
            }
...
void PrintMismatchV1(const char *KernelName, int MaxNumMismatch, float ErrorThreshold, half *CuBlas,
                     half *Other, int M_GLOBAL, int N_GLOBAL)
{
    printf("First 10 Mismatches between Cublas and %s:\n", KernelName);
    int count = 0;
    for (int i = 0; i < M_GLOBAL; i++)
    {
        for (int j = 0; j < N_GLOBAL; j++)
        {
            if (fabs(__half2float(CuBlas[i * N_GLOBAL + j]) -
                     __half2float(Other[i * N_GLOBAL + j])) > ErrorThreshold)
            {
                count++;
                printf("(%d,%d) CuBlas=%f %s=%f\n", i, j, __half2float(CuBlas[i * N_GLOBAL + j]),
                       KernelName, __half2float(Other[i * N_GLOBAL + j]));
            }
            if (count == MaxNumMismatch)
                break;
        }
...
void PrintMatrix(const char *MatrixName, half *Matrix, int M_GLOBAL, int N_GLOBAL, int MaxRows,
                 int MaxCols, int start_row, int start_col)
{
    printf("Printing Matrix %s (showing up to %d rows and %d columns, starting from row %d and "
           "column %d):\n",
...
void PrintMatrixVec(const char *MatrixName, half *Matrix, int M_GLOBAL, int N_GLOBAL, int MaxRows,
                    int MaxCols)
{
...
void PrintPerformance(const char *KernelName, float milliseconds, float tflops, double error)
{
    printf("%-10s \t -> \t\t Time/ms: %5.3f \t Performance/TFLOPs: %4.2f \t TotalError: %.2lf\n",
           KernelName, milliseconds, tflops, error);
}

void SavePerformanceData(const char *filename, int M, int K, int N, int SplitK, int Sparsity,
                         float duration_cublas_tc, float tflops_cublas_tc, float duration_SpMM2,
                         float tflops_SpMM2, float duration_SpMM_bitmapv3,
...
void SaveCuSparsePerformanceData(const char *filename, int M, int K, int N, int SplitK,
                                 int Sparsity, float duration_CuSparse_ColMajor,
                                 float tflops_CuSparse_ColMajor)
{
    FILE *fp;
    // Try to open file to check if it exists
    fp = fopen(filename, "r");
...
void printMatrix(const half *mat, int M, int K, int StartRow, int EndRow, int StartCol, int EndCol,
...
void printMatrixColumnMajor(const half *mat, int M, int K, int StartRow, int EndRow, int StartCol,
                            int EndCol, const std::string &name)
{
...
int InitSparseMatrixA_bitmap_v6(half *A_h, int M, int K,
                                int tile_M,        // 8
                                ...
                                half **Compressed_Val, int **TileOffsets, int **TileOffsets_median,
                                int **TileOffsets_global, uint64_t **bitmap, int &max_nnz_count)
{
    // Calc tile counts
    ...
    *TileOffsets_median = (int *)malloc(
        num_median_tiles * (tile_M_median / tile_M * tile_K_median / tile_K) * sizeof(int));
    *TileOffsets_global = (int *)malloc((num_global_tiles + 1) * sizeof(int));
    *bitmap = (uint64_t *)malloc(num_tiles * sizeof(uint64_t));

    if (*Compressed_Val == nullptr || *TileOffsets == nullptr || *TileOffsets_median == nullptr ||
        *TileOffsets_global == nullptr || *bitmap == nullptr)
    {
        return -1;
    }
    ...
    for (int global_tile_m = 0; global_tile_m < num_global_tiles_M; ++global_tile_m)
    {
        for (int global_tile_k = 0; global_tile_k < num_global_tiles_K; ++global_tile_k)
        {
            ...
            (*TileOffsets_median)[median_offset_idx++] =
                0; // The starting offset of each median tile is 0
            // Traverse the median tiles within the global tile (in row order)
            for (int median_tile_m = 0; median_tile_m < tile_M_global / tile_M_median;
                 ++median_tile_m)
            {
                for (int median_tile_k = 0; median_tile_k < tile_K_global / tile_K_median;
                     ++median_tile_k)
                {
                    ...
                    for (int local_tile_m_group = 0; local_tile_m_group < tile_M_median / tile_M;
                         local_tile_m_group += 2)
                    {
                        for (int local_tile_k_group = 0;
                             local_tile_k_group < tile_K_median / tile_K; local_tile_k_group += 2)
                        {
                                    ...
                                    for (int row_offset = 0; row_offset < tile_M; ++row_offset)
                                    {
                                        for (int col_offset = 0; col_offset < tile_K; ++col_offset)
                                        {
                                            ...
                                            if (row < M && col < K)
                                            {
                                                ...
                                                {
                                                    tile_bitmap |= (1ULL << (row_offset * tile_K +
                                                                             col_offset));
                    ...
                    if (median_tile_m < (tile_M_global / tile_M_median - 1) or
...
void print_bitmap_results(half *Compressed_Val, int *TileOffsets, uint64_t *bitmap, int num_tiles,
                          int val_count)
{
...
int InitSparseMatrixA_bitmap_v6_B(half *A_h, int M, int K,
                                  int tile_M,        // 8
                                  int tile_M_median, // 16
                                  int tile_M_global, // 64
                                  int tile_K,        // 8
                                  ...
                                  int tile_K_global, // 64
                                  half **Compressed_Val, int **TileOffsets,
                                  int **TileOffsets_median, int **TileOffsets_global,
                                  uint64_t **bitmap, int &max_nnz_count)
{
    // Calc tile counts
    int num_tiles_M = M / tile_M;
    int num_tiles_K = K / tile_K;
...
void print_bitmap_v3_results(half *Compressed_Val, int *TileOffsets, int *TileOffsets_global,
                             uint64_t *bitmap, int num_tiles, int num_global_tiles, int val_count)
{
...
void print_bitmap_v6_results(half *Compressed_Val, int *TileOffsets, int *TileOffsets_median,
                             int *TileOffsets_global, uint64_t *bitmap, int num_tiles,
                             int num_median_tiles, int num_global_tiles, int val_count)
{
    std::cout << "TileOffsets: ";
    for (int i = 0; i < num_tiles; ++i)
    {
