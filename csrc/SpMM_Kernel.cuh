                                      ...
                                      const int N_Global, const int K_Global, int Split_K)
{
    ...
    uint64_t *smem_Bitmap =
        reinterpret_cast<uint64_t *>(&smem[max_nnz_intile + (TILE_K * TilingConfig::TILE_N)]);
    ...
    const half *BTileGlobalPTR = B + Tile_Start_N * K_Global + BatchID * AverageNumKBlock * TILE_K;
    ...
    const uint64_t *BitmapTileGlobalPTR_B =
        bitmap_B + Tile_Start_Bitmap_B * K_Global +
        BatchID * AverageNumKBlock * TilingConfig::TILE_BITMAP_K_V3;
    ...
    for (int tile_id_k = 0; tile_id_k < NumIter; tile_id_k++)
    {
        ...
        CopyTileFromGlobalToShared_Bitmap_1_64<TilingConfig::TILE_BITMAP_M_V3, TilingConfig>(
            ...
            true); // Load the 2*8 bitmap after the double buffer B shared tile

        // Copying next B Tile to write shem
        CopyTileFromGlobalToShared_X_64<TilingConfig::TILE_N2, TilingConfig>(smem_B, BTileGlobalPTR,
                                                                             K_Global, true);
        ...
        CopyTileFromGlobalToShared_Sparse<TilingConfig>(
            smem, Compressed_A + current_sparse_tile_start, current_sparse_tile_nnz, true);
        CopyTileFromGlobalToShared_Sparse<TilingConfig>(
            smem_B_, Compressed_B + current_sparse_tile_start_B, current_sparse_tile_nnz_B, true);
        // __syncthreads();
        // // // 添加调试打印
        ...
        SpMM_LoadFragAwithBitmapFromShem(a, smem + TileOffsets_ThisWarp[(tile_id_k) * 4],
                                         smem_BitmapWarp, true);
        // SpMM_LoadFragAwithBitmapFromShem_B(b_, smem_B_, smem_Bitmap_B,
        // TileOffsets_ThisWarp, 0, true);

        PipelinedCoreComputationsBitmap<TilingConfig>(
            c, a, b, smem_B, warp_start_row, warp_start_col, smem_Bitmap_B, TileOffsets_ThisWarp_B,
    ...
    // memory.
    half *BlockGlobalPTR = Reduction_Workspace + BatchID * (M_Global * N_Global) + Tile_Start_M +
                           Tile_Start_N * M_Global;
