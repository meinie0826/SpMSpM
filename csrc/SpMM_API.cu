...
static void SpMM_SplitK_Kernel_Ex_bitmap_v3(cudaStream_t stream, const half *A, const half *Compressed_A, const int *TileOffsets,
    ...
    int dimN = max(N_Global / TilingConfig::TILE_N, 1); // max(N_Global/TilingConfig::TILE_N,1) used when N=8, TILE_N=16
    int dimM = M_Global / TilingConfig::TILE_M;
...
cudaError_t SpMM_SplitK_API_bitmap_v3(cudaStream_t stream, const half *A, const half *Compressed_A, const int *TileOffsets,
                                      const int *TileOffsets_Median, const uint64_t *bitmap, const int *max_nnz_intile, const half *B, half *C,
    ...
    switch (N_Global) {
    case 8:
        SpMM_SplitK_Kernel_Ex_bitmap_v3<TilingConfigBitmapV3<4, 1, 1, 1>>(stream, A, Compressed_A, TileOffsets, TileOffsets_Median, bitmap,
                                                                          max_nnz_intile, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global,
                                                                          Split_K);
        break;
    case 16:
        SpMM_SplitK_Kernel_Ex_bitmap_v3<TilingConfigBitmapV3<4, 1, 1>>(stream, A, Compressed_A, TileOffsets, TileOffsets_Median, bitmap,
                                                                       max_nnz_intile, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global,
                                                                       Split_K);
        break;
    case 32:
        SpMM_SplitK_Kernel_Ex_bitmap_v3<TilingConfigBitmapV3<4, 1, 2>>(stream, A, Compressed_A, TileOffsets, TileOffsets_Median, bitmap,
                                                                       max_nnz_intile, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global,
                                                                       Split_K);
        break;
    case 64:
        SpMM_SplitK_Kernel_Ex_bitmap_v3<TilingConfigBitmapV3<4, 1, 4>>(stream, A, Compressed_A, TileOffsets, TileOffsets_Median, bitmap,
                                                                       max_nnz_intile, B, SpMM_SplitK_OutputPTR, M_Global, N_Global, K_Global,
                                                                       Split_K);
        break;
    case 128:
...
__host__ int InitSparseMatrixA_bitmap(half *A_h, int M, int K,
                                      int tile_M,        // 8
...
extern "C" void Our_GenSparseMatrixBinFile(char *DenseMatrixFileName, int M, int K, char *Compressed_ValFileName,
                                           char *bitmap_TileOffsets_globalFileName, char *bitmap_TileOffsets_medianFileName, char *bitmapFileName,
                                           char *max_nnz_intileFileName, char *OutputSizesFileName) {
    ...
    auto num_gtilesv3 =
        InitSparseMatrixA_bitmap(host_array.data(), M, K, 8, 16, 64, 8, 64, 64, &Compressed_Val_cpu_v3, &bitmap_TileOffsets_cpu_v3,
    ...
    int val_count_v3 =
        bitmap_TileOffsets_global_cpu_v3[num_gtilesv3]; // The offset of the last tile is the total number of non - zero values after compression
    ...
    //         Write to FILE(bitmap_TileOffsets_globalFileName), size[num_gtilesv3 + 1], FILE(bitmap_TileOffsets_medianFileName), size[num_mtilesv3],
    //         FILE(BitmapFileName), size[num_ltilesv3] Write to FILE(max_nnz_intileFileName), size[1]
    ...
    if (!out_SizesFile.is_open() || !out_CompressedvalFile.is_open() || !out_BitmapglobalFile.is_open() || !out_BitmapmedianFile.is_open() ||
        !out_BitmapFile.is_open() | !out_NnzintileFile.is_open()) {
        printf("Our_GenSparseMatrixBinFile() ERROR: file %s, %s, %s, %s, %s or %s cannot be opened or creaetd. \n", OutputSizesFileName,
               Compressed_ValFileName, bitmap_TileOffsets_globalFileName, bitmap_TileOffsets_medianFileName, bitmapFileName, max_nnz_intileFileName);
        exit(-1);
    }
